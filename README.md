...
这是一个静态可视化选品系统，从家清个护、日化纸品方向的 40 个具体 SKU 池中，每天轮换展示 10 个候选。每个推荐项必须是单一品牌、单一规格、可追溯源商品链接的具体 SKU，并展示商品图片、三平台官方预估到手价、30 天销量、累计评价、建议售价、1688 单 SKU 最低价状态、预计毛利率和 1-10 分综合热度评分。
...
- 当前基础池为 40 个具体 SKU，覆盖纸品湿巾、衣物清洁、家庭清洁、个护日化；后续可继续扩容，但每日候选仍固定为 10 个。
- 每条推荐记录必须包含 `brand`、`name`、`sku`、`sourceUrl`、`image`、`platforms`、`suggestedPrice` 和评分输入。
- 不再使用“品牌方向”字段，不允许一条推荐里出现多个可选品牌。
- 只有品牌和全部规格词完全匹配时才标记“官方核验”；近似规格不参与价格和销量评分。
- 未取得官方接口数据时显示“待授权”或“无精确匹配”，不生成推测价格区间。
...
总分 = 跨平台热度 30% + 价格竞争力 20% + 利润可行性 20% + 官方 30 天销量 10% + 复购率 10% + 差异化空间 5% + 上架可操作性 5% - 风险扣分。

- 跨平台热度：淘宝/天猫、京东、抖音、小红书出现榜单、热词、内容种草或行业报告信号越多，得分越高。
- 价格竞争力：用“建议售价下限 ÷ 三平台有效最低预估到手价”计算，落在 95%-98% 时得分最高；无有效官方价格时按最低可信分处理。
- 利润可行性：有已核验的 1688 单 SKU 精确报价时按真实最低公开阶梯价计算；否则按 `建议售价 * 78%` 显示成本上限估算，并明确标记为非真实供货价。
- 官方 30 天销量：只使用联盟官方接口实际返回的销量字段；评价数、榜单和内容互动独立展示，不代替销量。
...
每日入选由“商业价值分 + 轮换分”共同决定。商业价值分保持原评分规则；轮换分额外考虑新鲜度、数据可信度、近 7 天出现次数、连续上榜和季节性，避免高分老 SKU 长期占满看板。每日目标结构为 2 个稳定跟踪、4 个轮换机会、3 个新发现、1 个季节机会，同时保持 3 个引流款、5 个平销款、2 个利润款。

页面顶部直接显示“今日替换数量、与昨日重合、近 7 日去重数”，候选区之后提供“持续观察”和“机会雷达”。这样既保留高质量 SKU 的连续跟踪，也让每天打开时有可见的新机会。
...
- 三平台官方市场缓存：`data/market_data.json`
...
系统每天依次调用淘宝联盟、京东联盟和抖音精选联盟官方 API。单个平台失败不会阻止其他平台刷新或 GitHub Pages 部署；失败时保留最近一次有效数据并标记老化状态。

- 24 小时内：新鲜，可参与价格和销量评分。
- 24-72 小时：老化，仍可参与评分但页面提示复核。
- 超过 72 小时：过期，只保留历史展示，退出价格评分。
- 官方预估到手价不等于逐笔实际成交价，可能不包含会员、地区、首购和定向补贴。

GitHub Actions 需要按实际开户情况配置以下 Secrets：

- 淘宝联盟：`TAOBAO_ALLIANCE_APP_KEY`、`TAOBAO_ALLIANCE_APP_SECRET`、`TAOBAO_ADZONE_ID`
- 京东联盟：`JD_UNION_APP_KEY`、`JD_UNION_APP_SECRET`、`JD_UNION_ID`
- 抖音精选联盟：`DOUYIN_CLIENT_KEY`、`DOUYIN_CLIENT_SECRET`、`DOUYIN_REFRESH_TOKEN`
- 抖音无人值守轮换：`MARKET_SECRETS_PAT`，使用具有仓库 Actions Secrets 读写权限的细粒度 PAT，仅用于把接口返回的新 `DOUYIN_REFRESH_TOKEN` 安全回写到仓库 Secret。

三家均未授权时，`python scripts/sync_market_data.py` 仍会成功完成，页面统一显示“待授权”。本地环境变量模板见 `.env.market.example`。

抖音的 refresh token 会轮换，旧 token 随后失效。云端刷新只把新 token 写入 GitHub runner 临时目录，回写 Secret 后立即删除，不写入仓库、缓存或日志。若未配置 `MARKET_SECRETS_PAT`，工作流会告警，需要在下一次刷新前手动更新 `DOUYIN_REFRESH_TOKEN`。

官方接口依据：

- [淘宝联盟物料搜索升级版](https://open.taobao.com/api.htm?docId=59529&docType=2)
- [京东联盟](https://union.jd.com/)
- [抖音精选联盟商品检索](https://op.jinritemai.com/docs/api-docs/61/1725)
- [抖店开放平台 API 调用与签名指南](https://op.jinritemai.com/docs/api/148)
- [抖店 token/refresh](https://op.jinritemai.com/docs/api-docs/162/1601)

## 1688 价格接入

系统只接受满足以下条件的 1688 报价：同一品牌、同一完整规格、单独 SKU、正数最低价、正数起订量、可访问的 `1688.com` 商品链接和核验时间。没有满足这些条件的数据时，页面显示“待核验”，不会生成虚假最低价。

云端每天先生成初步入选名单，再运行 `scripts/sync_1688_prices.py`，最后用最新报价重新生成并校验看板。首选数据源为淘宝开放平台官方接口 `alibaba.open.search.daixiao.offer.get`：按关键词搜索 1688 代销市场、按价格升序取回候选，然后只有标题同时命中品牌及全部规格词组的商品才会被认定为精确 SKU。
...
为适配每月 200 次的免费额度，云端默认每天优先刷新当日 Top3 和机会雷达前 2 个 SKU，共 5 次请求，月请求量约 150 次。其余 SKU 在进入重点名单时再复核，旧报价会保留核验时间：

- 48 小时内：`48小时内`
- 48 小时至 7 天：`需复核`
- 超过 7 天：`已过期`

在 GitHub Actions 手动运行时勾选 `full_refresh`，可一次刷新全部 40 个 SKU。该操作约消耗 40 次 API 请求，只适合首次接入或明确需要全池复核时使用。
...
脚本会隐藏输入内容，将密钥直接写入 GitHub Secret `ELIM_API_KEY`，随后触发全部 40 个 SKU 的首次刷新并等待 Pages 部署完成。密钥不会写入本地文件或 Git 历史；首次全量刷新约消耗 40 次 API 请求。
